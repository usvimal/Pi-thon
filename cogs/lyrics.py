...
class Lyrics(commands.Cog):
	...
	def __init__(self, bot):
		...
		self.bot = bot
		self.lyric_retriever = LyricsRetriever()
	...
	async def source(self, ctx):
		await ctx.send("Current lyric source is {}.".format(LyricsRetriever.get_main_source(self)))

	@lyrics.command()
	async def change_source(self, ctx, new_source):
		try:
			LyricsRetriever.change_main_source(self, new_source)
	...
	async def show_lyrics_from_description(self, ctx, song_title, song_artist):
		...
		for chunk in chunks(LyricsRetriever.get_lyrics(self, song_title, song_artist), 2048):
